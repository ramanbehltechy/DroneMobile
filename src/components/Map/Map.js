...
import { View, PermissionsAndroid, StyleSheet, Alert, ActivityIndicator } from 'react-native';
...
const Map = () => {
  ...
  const getLocation = () => {
    Geolocation.getCurrentPosition(
      position => {
        ...
        fetchNearbyLocations(latitude, longitude, timeZone ? timeZone : 'PST');
      },
      error => {
  ...
  const getDistance = (coord1, coord2) => {
    ...
    const radius = distance * 1000 + 100; // Convert to meters
    return radius;
  };
        ...
        currentLocation ? (
          ...
          /> */}
            {nearByLocations?.map((location, index) => {
              let showTime = new Date(location?.utcDateTime);
